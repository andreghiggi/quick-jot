...
- Regra: com `mercadoEnabled=true`, `ncm` e `cest` podem vir do cadastro do produto se preenchidos; senão vêm da regra tributária; senão fallback ('00000000' / vazio). **`cfop` SEMPRE vem da regra tributária; o campo CFOP do cadastro do produto é ignorado na NFC-e.** Fallback de CFOP sem regra: '5102'. Demais campos (CSOSN, alíquotas, CSTs) **sempre** da regra.
...
- 8+ call sites usam o helper: `PDV.tsx` (2), `PDVV2.tsx` (2), `OrderCardChargeDialog.tsx` (2), `PedidoExpressDialog.tsx` (2), `FrenteCaixa.tsx` (1), `FrenteCaixaLista.tsx` (1), `Receitas.tsx` (1). Lojas sem módulo mercado: comportamento idêntico ao anterior.
