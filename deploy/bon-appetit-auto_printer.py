...
def _prepare_pywin32_dll_path(verbose=False):
    """Windows 11: pywin32_system32 no PATH antes de import win32print/win32ui."""
    try:
        script_dir = os.path.dirname(os.path.abspath(__file__))
        if script_dir not in sys.path:
            sys.path.insert(0, script_dir)
        try:
            from verificar_pywin32 import prepare_pywin32_dll_path

            prepare_pywin32_dll_path(verbose=verbose)
            return True
        except ImportError:
            pass
        ...
        for raw in roots:
            ...
            if key in seen or not root.exists():
                continue
            seen.add(key)
            for sub in ("pywin32_system32", "win32", "win32/lib", "Pythonwin"):
                candidates.append(root / sub.replace("/", os.sep))
...
STORE_INFO = {}
COMPANY_ID = "32b71649-461d-4cb6-b26c-12390b090feb" # Injetado pelo frontend
...
def extrair_blocos_v2(html_content):
    ...
    # Recibo rico V39 (.store-name / .order-num — pdvV2Print buildReceiptHtmlV2Rich)
    ...
    if store_nodes or order_nodes:
        ...
        for tl in by_class("total-line"):
            spans = [c.text() for c in tl.children if c.tag == "span"]
            if len(spans) >= 2:
                blocos.append(block(spans[0], "total", "left", spans[1]))

        for gt in by_class("grand-total"):
            spans = [c.text() for c in gt.children if c.tag == "span"]
            if len(spans) >= 2:
                blocos.append(block(spans[0], "total", "left", spans[1]))
...
def imprimir_gdi(printer_name, conteudo, largura_mm=None):
    ...
    """
    try:
        win32ui, win32con = _import_win32ui()
...
def _imprimir_html(html_content, station_id=None):
    ...
    try:
        ...
        try:
            ...
            win32ui_ok = False
            log(f"win32ui indisponivel — usando modo RAW: {ui_err}", "AVISO")
        from html.parser import HTMLParser
