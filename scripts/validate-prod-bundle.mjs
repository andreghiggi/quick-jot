...
/**
 * Valida que o bundle em app.comandatech.com.br aponta para VPS API.
 *
...
const vpsEndpoint = /https:\/\/api\.comandatech\.com\.br/.test(js);
...
if (lovableCount > 0) {
  console.error('FAIL: bundle ainda aponta para Lovable Cloud (indisponível)');
  process.exit(1);
}
if (!vpsEndpoint) {
  console.error('FAIL: bundle não contém api.comandatech.com.br');
  process.exit(1);
}
if (/Use Supabase Cloud|API self-hosted/.test(js)) {
  console.error('FAIL: bundle ainda bloqueia VPS no client.ts');
  process.exit(1);
}
...
console.log(`  auth API VPS: HTTP ${authRes.status} (OK)`);
console.log('PASS: produção aponta para VPS API');
