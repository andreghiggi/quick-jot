            ...
            <div className="space-y-2">
              {categories.map((cat, index) => (
                <div key={cat.id} className={cn("flex items-center justify-between p-3 border rounded-lg bg-background", cat.active === false && "opacity-60")}>
                  ...
                  {stations.length > 0 && (
                    <Select
                      value={categoryMappings[cat.id] ?? '_default'}
                      onValueChange={(v) =>
                        void mapCategoryToStation(cat.id, v === '_default' ? null : v)
                      }
                    >
                  ...
                  <div className="flex items-center gap-1 flex-shrink-0">
                    ...
                    <div className="relative group">
                      <Button
                        variant="ghost"
                        size="sm"
                        className={cn("h-8 w-8 p-0", cat.waiterItem === false && "text-muted-foreground/40")}
                        onClick={() => updateCategory(cat.id, { waiterItem: cat.waiterItem === false })}
                      >
                        <UtensilsCrossed className="h-3.5 w-3.5" />
                      </Button>
                      <span className="absolute bottom-full left-1/2 -translate-x-1/2 mb-1 px-2 py-1 text-xs text-white bg-gray-800 rounded whitespace-nowrap opacity-0 group-hover:opacity-100 transition-opacity pointer-events-none z-50">
                        {cat.waiterItem === false ? 'Oculta no Garçom' : 'Visível no Garçom'}
                      </span>
                    </div>
                    ...
                    <div className="relative group">
                      <Button
                        variant="ghost"
                        size="sm"
                        className={cn("h-8 w-8 p-0", cat.active === false ? "text-destructive" : "text-green-600")}
                        onClick={() => updateCategory(cat.id, { active: !(cat.active !== false) })}
                      >
                        <Power className="h-3.5 w-3.5" />
                      </Button>
                      <span className="absolute bottom-full left-1/2 -translate-x-1/2 mb-1 px-2 py-1 text-xs text-white bg-gray-800 rounded whitespace-nowrap opacity-0 group-hover:opacity-100 transition-opacity pointer-events-none z-50">
                        {cat.active === false ? 'Categoria inativa — clique para ativar' : 'Categoria ativa — clique para inativar'}
                      </span>
                    </div>
                    <div className="relative group">
                      <Button
